	...
	return (
		<div className="space-y-4">
			...
			>
				{/* Days of week header */}
				<div
					className={`grid grid-cols-7 border-b ${isDarkMode ? "border-slate-600" : "border-gray-200"}`}
				>
				...
				<div className="grid grid-cols-7">
					{days.map((dayObj, index) => {
						...
						const isToday =
							today.getFullYear() === dayObj.date.getFullYear() &&
							...
							>
								...
								{entry && (
									<div className="mt-1 flex space-x-1">
										<div
											className={`w-2 h-2 rounded-full ${qualityToColor[entry.sleepQuality]}`}
										></div>
									</div>
								)}
			...
			>
				<h4
					className={`text-sm font-medium mb-2 ${isDarkMode ? "text-slate-300" : "text-gray-700"}`}
				>
				...
				<div className="flex flex-wrap gap-3 text-xs">
					{Object.entries(qualityToColor).map(([quality, color]) => (
						<div key={quality} className="flex items-center space-x-1">
							...
							<span
								className={`capitalize ${isDarkMode ? "text-slate-400" : "text-gray-600"}`}
							>
								{quality}
							</span>
						</div>
			...
			{selectedEntry && (
				<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
					<div
						className={`rounded-lg p-6 max-w-md w-full max-h-96 overflow-y-auto ${
							isDarkMode
						...
						<div className="space-y-3">
							<div className="grid grid-cols-2 gap-4 text-sm">
								<div>
									...
									<div className="flex items-center space-x-2">
										<div
											className={`w-3 h-3 rounded-full ${qualityToColor[selectedEntry.sleepQuality]}`}
										></div>
								...
								<div>
									...
									<div className="font-medium">
										{selectedEntry.timeToFallAsleep.replace("-", " ")}
									</div>
								</div>
								<div>
									<span
							...
							{selectedEntry.notes && (
								<div>
									<span
										className={`block text-sm ${isDarkMode ? "text-slate-400" : "text-gray-500"}`}
									>
										Notes:
									</span>
