...
const sleepOnsetOptions = [
	{
		...
		description: "Fell asleep very quickly",
	},
	{
		value: "10-20",
		label: "10-20 minutes",
		description: "Normal, healthy sleep onset",
	},
	{
		value: "20-30",
		label: "20-30 minutes",
		description: "Took a while but acceptable",
	},
	{
		value: "30-60",
		label: "30-60 minutes",
		description: "Too long, investigate causes",
	},
	{
		value: "over-60",
		label: "Over 60 minutes",
		description: "Significant sleep problem",
	},
] as const;
...
const ExpandableQuestion = ({
	...
	return (
		<div
			className={`border rounded-lg ${isDarkMode ? "border-slate-600" : "border-gray-300"}`}
		>
			<button
				type="button"
			...
			>
				<div className="flex items-center space-x-3">
					...
					{hasValue && (
						<span
							className={`text-sm capitalize px-2 py-1 rounded ${isDarkMode ? "bg-slate-700 text-slate-300" : "bg-gray-100 text-gray-600"}`}
						>
							{value.replace("-", " ")}
						</span>
					)}
			...
			{isExpanded && (
				<div
					className={`p-4 border-t ${isDarkMode ? "border-slate-600 bg-slate-800" : "border-gray-200 bg-gray-50"}`}
				>
					...
					{value && (
						<button
							...
							onClick={() => {
								onSave({
									[name]: value as SleepQuality | EnergyLevel | SleepOnsetTime,
								});
...
export default function DailyTrackingForm({
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		if (
			...
			!formData.morningEnergy ||
			!formData.timeToFallAsleep ||
			!formData.afternoonEnergy
		) {
		...
		const entry: SleepEntry = {
			...
			...(formData.preBedtimeActivities && {
				preBedtimeActivities: formData.preBedtimeActivities,
			}),
				...
				<div className="space-y-4">
					<ExpandableQuestion
						...
						onChange={(value) =>
							setFormData((prev) => ({
								...prev,
								sleepQuality: value as SleepQuality,
							}))
						}
						name="sleepQuality"
					...
					<ExpandableQuestion
						...
						onChange={(value) =>
							setFormData((prev) => ({
								...prev,
								morningEnergy: value as EnergyLevel,
							}))
						}
						name="morningEnergy"
						onSave={saveIndividualAnswer}
					...
					<ExpandableQuestion
						...
						onChange={(value) =>
							setFormData((prev) => ({
								...prev,
								timeToFallAsleep: value as SleepOnsetTime,
							}))
						}
					...
					<ExpandableQuestion
						...
						onChange={(value) =>
							setFormData((prev) => ({
								...prev,
								afternoonEnergy: value as EnergyLevel,
							}))
						}
				...
				>
					...
					>
						<div className="flex items-center space-x-3">
							...
							{formData.notes && (
								<span
									className={`text-sm px-2 py-1 rounded ${isDarkMode ? "bg-slate-700 text-slate-300" : "bg-gray-100 text-gray-600"}`}
								>
									Added
								</span>
							)}
						...
						<span
							className={`transform transition-transform ${expandedQuestions.notes ? "rotate-180" : ""}`}
						>
							↓
						</span>
					</button>
					...
					>
						...
						<div>
							...
							<input
								...
								onChange={(e) =>
									setFormData((prev) => ({ ...prev, roomTemp: e.target.value }))
								}
								className={`w-full p-2 rounded border ${
									isDarkMode
										? "bg-slate-600 border-slate-500 text-slate-200"
