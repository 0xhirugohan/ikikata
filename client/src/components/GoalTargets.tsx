...
const goals = {
	sleepQuality: {
		...
		description: '"Good" or "Excellent" most nights',
	},
	morningEnergy: {
		target: "alert-energized",
		description: '"Alert" or "Energized" most mornings',
	},
	sleepOnset: {
		target: "under-20",
		description: "Under 20 minutes consistently",
	},
	afternoonEnergy: {
		target: "alert-energized",
		description: '"Alert" or "Energized" (minimal crash)',
	},
};
...
export default function GoalTargets({ entries, isDarkMode }: Props) {
	const calculateGoalProgress = () => {
		...
		const sleepQualityGoal =
			(recentEntries.filter((entry) => entry.sleepQuality === "good").length /
				recentEntries.length) *
			100;

		const morningEnergyGoal =
			(recentEntries.filter(
				(entry) =>
					entry.morningEnergy === "alert" ||
					entry.morningEnergy === "energized",
			).length /
				recentEntries.length) *
			100;

		const sleepOnsetGoal =
			(recentEntries.filter(
				(entry) =>
					entry.timeToFallAsleep === "under-10" ||
					entry.timeToFallAsleep === "10-20",
			).length /
				recentEntries.length) *
			100;

		const afternoonEnergyGoal =
			(recentEntries.filter(
				(entry) =>
					entry.afternoonEnergy === "alert" ||
					entry.afternoonEnergy === "energized",
			).length /
				recentEntries.length) *
			100;
	...
	const overallScore =
		(progress.sleepQuality +
			progress.morningEnergy +
			progress.sleepOnset +
			progress.afternoonEnergy) /
		4;
	...
	const getRecommendationsForGoals = () => {
		...
		if (progress.sleepQuality < 60) {
			recommendations.push(
				"Focus on improving sleep quality by optimizing your sleep environment (temperature, darkness, noise)",
			);
		}

		if (progress.morningEnergy < 60) {
			recommendations.push(
				"Low morning energy may indicate insufficient sleep duration or poor sleep quality",
			);
		}

		if (progress.sleepOnset < 60) {
			recommendations.push(
				"Taking too long to fall asleep? Try establishing a consistent pre-bedtime routine",
			);
		}

		if (progress.afternoonEnergy < 60) {
			recommendations.push(
				"Afternoon energy crashes might be related to sleep debt or inconsistent sleep schedule",
			);
		}

		if (recommendations.length === 0) {
			recommendations.push(
				"Congratulations! You're meeting most of your sleep health goals. Keep up the great work!",
			);
		}
		...
		return (
			<div className="p-8 text-center">
				...
				<p className={isDarkMode ? "text-slate-400" : "text-gray-600"}>
					Start tracking your sleep to see your progress toward healthy sleep
					goals.
				</p>
			</div>
		);
	...
	return (
		<div className="p-6 space-y-8">
			<div>
				...
				>
					Track your progress toward healthy sleep indicators based on{" "}
					{Math.min(entries.length, 21)} recent entries
				</p>
			</div>

			<div
				className={`p-6 rounded-lg text-center ${isDarkMode ? "bg-slate-700" : "bg-gray-50"}`}
			>
			...
			<div className="grid md:grid-cols-2 gap-6">
				...
				>
					...
					<div
						className={`h-4 bg-gray-200 rounded-full overflow-hidden mb-3 ${isDarkMode ? "bg-gray-600" : ""}`}
					>
						<div
			...
			>
				...
				<div className="space-y-3">
					<div className="flex items-center space-x-3">
						<span
							className={`text-xl ${progress.sleepQuality >= 80 ? "✅" : "⏳"}`}
						>
							{progress.sleepQuality >= 80 ? "✅" : "⏳"}
						</span>
						<div>
							<span className="font-medium">
								Sleep Quality: "Good" or "Excellent" most nights
							</span>
							<div
								className={`text-sm ${isDarkMode ? "text-slate-400" : "text-gray-600"}`}
							>
					...
					<div className="flex items-center space-x-3">
						<span
							className={`text-xl ${progress.morningEnergy >= 80 ? "✅" : "⏳"}`}
						>
							{progress.morningEnergy >= 80 ? "✅" : "⏳"}
						</span>
						<div>
							<span className="font-medium">
								Morning Energy: "Alert" or "Energized" most mornings
							</span>
							<div
								className={`text-sm ${isDarkMode ? "text-slate-400" : "text-gray-600"}`}
							>
					...
					<div className="flex items-center space-x-3">
						<span
							className={`text-xl ${progress.sleepOnset >= 80 ? "✅" : "⏳"}`}
						>
							{progress.sleepOnset >= 80 ? "✅" : "⏳"}
						</span>
						<div>
							<span className="font-medium">
								Time to Fall Asleep: Under 20 minutes consistently
							</span>
							<div
								className={`text-sm ${isDarkMode ? "text-slate-400" : "text-gray-600"}`}
							>
					...
					<div className="flex items-center space-x-3">
						<span
							className={`text-xl ${progress.afternoonEnergy >= 80 ? "✅" : "⏳"}`}
						>
							{progress.afternoonEnergy >= 80 ? "✅" : "⏳"}
						</span>
						<div>
							<span className="font-medium">
								Afternoon Energy: "Alert" or "Energized" (minimal crash)
							</span>
							<div
								className={`text-sm ${isDarkMode ? "text-slate-400" : "text-gray-600"}`}
							>
