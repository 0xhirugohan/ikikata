...
class Database {
   ...
   */
  async createAccount(publicKey: string): Promise<{ success: boolean; accountId?: string; error?: string }> {
    try {
      // Import and validate the public key
      const crypto = await import('./crypto');
      const importedKey = await crypto.importPublicKey(publicKey);
      const accountId = await crypto.generateAccountIdFromPublicKey(importedKey);
   ...
   */
  storeChallenge(accountId: string, challenge: string, expiresAt: number): void {
   ...
   */
  addSleepEntry(accountId: string, entry: Omit<SleepEntry, 'accountId' | 'createdAt'>): boolean {
   ...
   * Update sleep entry
   */
  updateSleepEntry(accountId: string, entryId: string, updates: Partial<SleepEntry>): boolean {
    const entries = this.sleepEntries.get(accountId);
    if (!entries) return false;
  ...
  getStats() {
    return {
      ...
      totalSleepEntries: Array.from(this.sleepEntries.values()).reduce((sum, entries) => sum + entries.length, 0)
    };
  }
}
