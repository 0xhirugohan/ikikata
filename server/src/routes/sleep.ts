...
sleep.get("/analytics", async (c) => {
	try {
		...
		const avgSleepQuality =
			entries.reduce(
				(sum, entry) => sum + sleepQualityMap[entry.sleepQuality],
				0,
			) / entries.length;

		const avgMorningEnergy =
			entries.reduce(
				(sum, entry) => sum + energyLevelMap[entry.morningEnergy],
				0,
			) / entries.length;

		const avgAfternoonEnergy =
			entries.reduce(
				(sum, entry) => sum + energyLevelMap[entry.afternoonEnergy],
				0,
			) / entries.length;

		const avgSleepOnset =
			entries.reduce(
				(sum, entry) => sum + sleepOnsetMap[entry.timeToFallAsleep],
				0,
			) / entries.length;
